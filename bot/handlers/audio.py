...
def register_audio_handlers(app: Client):
    ...
    async def handle_audio(client: Client, message: Message):
        ...
        try:
            ...
            # Спрашиваем тип контента
            ctx_type = get_user_context_type(message.from_user.id)
            if ctx_type == "meeting":  # default — спросить
...
async def _run_pipeline(
    ...
    try:
        ...
        # 7. Формируем краткое резюме для чата
        summary_text = _format_chat_summary(expert_data, topics, diag, lang)
        ...
        # Cleanup
        for p in [audio_path, pdf_path if 'pdf_path' in dir() else None,
                  html_path if 'html_path' in dir() else None]:
...
def _format_chat_summary(expert_data: dict, topics: dict, diag: dict, lang: str) -> str:
